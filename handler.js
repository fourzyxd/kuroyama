    ...
    try {
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          m.plugin = name
          if (name === 'menu.js' && (m.sender === '6285748600473@s.whatsapp.net' || m.sender === '6285748600473@s.whatsapp.net')) m.reply('Hai Dev!')
          ...
          //if(!user.owner || !user.rowner) return conn.reply(m.chat, `Maaf @${m.sender.split`@`[0]} \n*『 Info 』*\nKami Staff Kuroyama ingin melakukan backup data, dan sedikit meningkatkan performa bot.\nJadi, Kami dari Staff Kuroyama memutuskan untuk melakukan *maintenance*, jika ada yang mau ditanyakan, harap hubungi Real Owner, Terima Kasih\n\n_*Hormat Kami, Staff Kuroyama*_`, m, { mentions: [m.sender] })
          ...
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.sendButton(m.chat, `Limit anda habis, silahkan chat Owner atau beli melalui *${usedPrefix}buy*`, '© kuroyama.id', { 'button[0]': 'Owner', 'row[0]': '.owner', 'button[1]': 'Buy', 'row[1]': '.buy', 'button[2]': 'Buy All', 'row[2]': '.buyall' }, m)
...
global.dfail = (type, m, conn) => {
  ...
  global.dfails = (jid, buffer, content, footer, button1, row1, button2, row2, quoted) => {
          ...
          Hormat kami Staff fourzy Bot
          `, '©fourzy Bot', 'Terima', '.terima', 'Tidak', '.tidak', m)
